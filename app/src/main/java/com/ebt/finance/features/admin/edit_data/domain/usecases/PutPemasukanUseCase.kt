...
class PutPemasukanUseCase @Inject constructor(
    private val repository: UpdateDataRepository
) {
    operator fun invoke(token: String, id: String, data: TambahData): Flow<Resource<Failed>> = flow {
        emit(Resource.Loading())
        try {
            val result = repository.updatePemasukan(
                token,
                id,
                data
            )
